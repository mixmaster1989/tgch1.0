...
class PineSlicerApp(tk.Tk):
    ...
    def load_presets(self):
        ...
        if os.path.exists("presets.json"):
            try:
                ...
                messagebox.showerror("Ошибка", f"Не удалось загрузить пресеты: {str(e)}")
        return presets

    def get_categories(self):
        return list(set(block["category"] for block in BLOCKS))

    def load_last_session(self):
        # Здесь можно реализовать загрузку последней сессии из файла
        pass
    ...
    def get_selected_blocks(self):
        ...
        for block_key, var in self.block_vars.items():
            if var.get():
                selected_block = next((block for block in BLOCKS if block["key"] == block_key), None)
                if selected_block:
    ...
    def filter_blocks_by_category(self, event=None):
        ...
        self.render_blocks(category if category != "Все" else None)

    def search_blocks(self, *args):
        search_term = self.search_var.get().lower()
        self.render_blocks(search_term=search_term if search_term else None)

    def render_blocks(self, category=None, search_term=None):
        # Очищаем текущие блоки
        for widget in self.blocks_frame_inner.winfo_children():
            widget.destroy()
        
        # Фильтруем блоки по категории и поиску
        filtered_blocks = BLOCKS
        if category:
            filtered_blocks = [block for block in filtered_blocks if block["category"] == category]
        if search_term:
            filtered_blocks = [block for block in filtered_blocks if search_term in block["label"].lower() or search_term in block["description"].lower()]
        
        # Создаем чекбоксы для каждого блока
        for block in filtered_blocks:
            var = tk.BooleanVar()
            chk = ttk.Checkbutton(self.blocks_frame_inner, text=block["label"], variable=var)
            chk.pack(anchor="w", padx=5, pady=2)
            self.block_vars[block["key"]] = var
            
            # Привязываем событие изменения состояния
            var.trace_add("write", lambda *args, b=block["key"]: self.update_block_params(b))
        
        # Обновляем параметры для уже выбранных блоков
        self.update_selected_block_params()

    def update_block_params(self, block_key):
        # Получаем состояние чекбокса
        is_selected = self.block_vars[block_key].get()
        
        # Находим блок в списке BLOCKS
        block = next((b for b in BLOCKS if b["key"] == block_key), None)
        if not block:
            return
        
        # Если блок не выбран, удаляем его параметры
        if not is_selected:
            for param in block["params"]:
                if param["name"] in self.block_param_vars:
                    del self.block_param_vars[param["name"]]
            return
        
        # Добавляем параметры блока
        for param in block["params"]:
            if param["type"] == "int":
                default_value = param.get("default", 14)
                if isinstance(default_value, str):
                    try:
                        default_value = int(default_value)
                    except ValueError:
                        default_value = 14
                self.block_param_vars[param["name"]] = tk.IntVar(value=default_value)
            elif param["type"] == "float":
                default_value = param.get("default", 14.0)
                if isinstance(default_value, str):
                    try:
                        default_value = float(default_value)
                    except ValueError:
                        default_value = 14.0
                self.block_param_vars[param["name"]] = tk.DoubleVar(value=default_value)
            elif param["type"] == "bool":
                default_value = param.get("default", True)
                self.block_param_vars[param["name"]] = tk.BooleanVar(value=default_value)
            elif param["type"] == "color":
                default_value = param.get("default", "color.blue")
                self.block_param_vars[param["name"]] = tk.StringVar(value=default_value)
            elif param["type"] == "combo":
                default_value = param.get("default", param["options"][0][1] if param["options"] else "")
                self.block_param_vars[param["name"]] = tk.StringVar(value=default_value)
        
        # Обновляем отображение параметров
        self.update_params_display()

    def update_selected_block_params(self):
        # Обновляем параметры для всех выбранных блоков
        for block_key, var in self.block_vars.items():
            if var.get():
                self.update_block_params(block_key)

    def update_params_display(self):
        # Очищаем текущие виджеты параметров
        for widget in self.param_frame_inner.winfo_children():
            widget.destroy()
        
        # Создаем новые виджеты для отображения параметров
        row = 0
        for param, var in self.block_param_vars.items():
            # Находим параметр в BLOCKS
            block = None
            param_def = None
            for b in BLOCKS:
                p = next((p for p in b["params"] if p["name"] == param), None)
                if p:
                    block = b
                    param_def = p
                    break
            
            if not block or not param_def:
                continue
            
            ttk.Label(self.param_frame_inner, text=param_def["label"] + ":").grid(row=row, column=0, sticky="w", padx=5, pady=2)
            
            if param_def["type"] == "int" or param_def["type"] == "float":
                entry = ttk.Entry(self.param_frame_inner, textvariable=var, width=10)
                ...
                frame = ttk.Frame(self.param_frame_inner)
                frame.grid(row=row, column=1, sticky="w", padx=5, pady=2)
                
                # Поле с цветом
                color_entry = ttk.Entry(frame, textvariable=var, width=15)
                color_entry.pack(side=tk.LEFT, padx=(0, 5))
                
                # Кнопка выбора цвета
                def create_color_picker(var):
                    def pick_color():
                        color = colorchooser.askcolor(title=f"Выберите цвет для {param_def['label']}", initialcolor=var.get())
                        if color[1]:
                            var.set(f"color.new({color[1]}, 0)" if "transparency" not in param_def else f"color.new('{color[1]}', {var.get().split(',')[-1].strip(')')})")
                
                pick_button = ttk.Button(frame, text="...", command=create_color_picker(var), width=3)
                pick_button.pack(side=tk.LEFT)
            elif param_def["type"] == "combo":
                combo = ttk.Combobox(self.param_frame_inner, textvariable=var, state="readonly", width=15)
                if isinstance(param_def["options"], list):
                    combo["values"] = param_def["options"] if isinstance(param_def["options"][0], str) else [opt[1] for opt in param_def["options"]]
                    combo.current(0)
    ...
    def setup_syntax_highlighting(self):
        # Базовая подсветка синтаксиса для Pine Script
        self.text_editor.tag_configure("keyword", foreground="#0000FF")  # Синий
        self.text_editor.tag_configure("function", foreground="#008000")  # Зеленый
        self.text_editor.tag_configure("comment", foreground="#808080")   # Серый
        self.text_editor.tag_configure("string", foreground="#A31515")    # Красный
        self.text_editor.tag_configure("number", foreground="#098658")     # Зеленый
        
        # Пример ключевых слов и функций Pine Script
        self.keywords = {"indicator", "plot", "ta.", "input.", "alertcondition", "bgcolor", "plotshape"}
        self.functions = {"ema", "sma", "rsi", "stoch", "macd", "bb", "volume", "channel"}
        
        # Привязываем событие изменения текста к подсветке
    ...
    def highlight_syntax(self, event=None):
        ...
        self.text_editor.tag_remove("function", "1.0", "end")
        ...
        for i, line in enumerate(lines):
            ...
            if comment_pos != -1:
                self.text_editor.tag_add("comment", f"{i+1}.{comment_pos}", f"{i+1}.end")
    def highlight_syntax(self, event=None):
...
if __name__ == "__main__":
    ...
    def load_preset(self):
        preset_name = self.preset_var.get()
        if not preset_name:
            messagebox.showwarning("Предупреждение", "Выберите пресет для загрузки")
            return
        
        if preset_name not in self.presets:
            messagebox.showerror("Ошибка", f"Пресет '{preset_name}' не найден")
            return
        ...
        preset_blocks = self.presets[preset_name]["blocks"]
        preset_params = self.presets[preset_name]["params"]
        
        # Сначала устанавливаем состояние чекбоксов
        for block_key, var in self.block_vars.items():
            is_selected = block_key in preset_blocks
            var.set(is_selected)
            
            # Обновляем параметры блока
            if is_selected:
                for param_name, value in preset_params.items():
                    if param_name in self.block_param_vars:
                        self.block_param_vars[param_name].set(value)
        
        # Пересоздаем виджеты параметров
        self.update_params_display()
        
        # Сохраняем текущий пресет как последний использованный
