...
class Active5050Balancer:
    ...
    def __init__(self):
        ############################################################
        # ⚙️ ИНИЦИАЛИЗАЦИЯ
        # Назначение: клиенты API, параметры, лимиты, статистика
        ############################################################
        self.mex_api = MexAPI()
    ...
    ############################################################
    # 🔄 ОБЕСПЕЧЕНИЕ USDC (конвертация из USDT)
    ############################################################
    ...
    ############################################################
    # 🛡️ ПРОВЕРКА БЕЗОПАСНОСТИ ОПЕРАЦИИ
    ############################################################
    ...
    ############################################################
    # 💹 СТОИМОСТЬ ПОРТФЕЛЯ (АЛЬТЫ vs BTC/ETH)
    ############################################################
    ...
    ############################################################
    # 🧾 ИСПОЛНЕНИЕ ОПЕРАЦИИ БАЛАНСИРОВКИ
    ############################################################
    ...
    async def buy_btceth_with_usdc(self, amount: float) -> bool:
        ...
        try:
            ...
            # Конвертация USDT → USDC выполнена успешно (уведомление убрано для уменьшения спама)
            logger.info(f"✅ USDC подготовлен для покупки BTC/ETH: ${amount:.2f} USDC")
