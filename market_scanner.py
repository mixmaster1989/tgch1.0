        ...
        try:
            ...
            for attempt, method_info in enumerate(rounding_methods, 1):
                try:
                    ...
                    # Отправляем уведомление о попытке
                    attempt_message = (
                        f"🔄 <b>ПОПЫТКА {attempt}/6</b>\n\n"
                        f"📈 <b>{symbol}</b>\n"
                        f"💰 Сумма: ${usdt_amount:.2f} USDT\n"
                        f"📊 Метод: {method_name}\n"
                    ...
                    self.send_telegram_message(attempt_message)
                    ...
                    if 'orderId' in order:
                        ...
                        # Отправляем уведомление об ошибке
                        error_message = (
                            f"❌ <b>ПОПЫТКА {attempt}/6 НЕ УДАЛАСЬ</b>\n\n"
                            f"📈 <b>{symbol}</b>\n"
                            f"📊 Метод: {method_name}\n"
                            f"📈 Количество: {quantity}\n\n"
                            f"🚫 <b>ОШИБКА:</b>\n"
                    ...
                    exception_message = (
                        f"💥 <b>ИСКЛЮЧЕНИЕ В ПОПЫТКЕ {attempt}/6</b>\n\n"
                        f"📈 <b>{symbol}</b>\n"
                        f"📊 Метод: {method_name}\n\n"
                        f"🚫 <b>ОШИБКА:</b>\n"
                        f"{str(e)}\n\n"
                    ...
                    self.send_telegram_message(exception_message)
            ...
            self.send_telegram_message(final_error_message)
